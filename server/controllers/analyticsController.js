  ...
  if (periodType === "yearly") {
    for (let month = 0; month < 12; month += 1) {
      seed.push({
        ...
        label: new Date(Date.UTC(start.getUTCFullYear(), month, 1)).toLocaleString(
          "en-IN",
          {
            month: "short",
            timeZone: "UTC",
...
async function getDashboardAnalytics(user, query = {}) {
  await connectDb();

  const periodType = String(query.periodType || "monthly").toLowerCase();
  if (!ALLOWED_PERIOD_TYPES.includes(periodType)) {
    throw new AppError(
      `periodType must be one of: ${ALLOWED_PERIOD_TYPES.join(", ")}`,
      400,
    );
  }
  ...
  const savingsRate =
    totals.income > 0 ? Number(((totals.savings / totals.income) * 100).toFixed(2)) : 0;
